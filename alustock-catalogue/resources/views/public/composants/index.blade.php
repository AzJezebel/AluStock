...
<div>
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-aluminum-900">Tous les composants</h1>
        <p class="text-aluminum-500 text-sm mt-1">Découvrez l'ensemble de nos composants techniques.</p>
    </div>
    ...
    @if($composants->count())
                    ...
                    <div class="p-5">
                        ...
                        <div class="flex items-center justify-between mt-4 pt-3 border-t border-aluminum-100">
                            <span class="text-xs text-aluminum-400">
                                {{ $composant->created_at?->format('d/m/Y') ?? 'N/A' }}
                            </span>
                            <a href="{{ route('composants.show', $composant->slug) }}" 
        ...
        <div class="mt-6">
            {{ $composants->appends(request()->query())->links() }}
        </div>
    @else
        <div class="text-center py-12 bg-white rounded-xl border border-aluminum-200">
            <p class="text-aluminum-500">Aucun composant disponible.</p>
        </div>
    @endif
</div>
@endsection