{{-- resources/views/admin/gammes/edit.blade.php --}}

@extends('admin.layouts.admin')

@section('title', 'Modifier la gamme - AluStock Admin')
@section('page-title', 'Modifier la gamme')
@section('page-subtitle', 'Mettre à jour la gamme')

@section('content')
<div class="bg-white rounded-lg shadow-sm p-6">
    <form action="{{ route('admin.gammes.update', $gamme) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="nom" name="nom" value="{{ old('nom', $gamme->nom) }}" required
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4"
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500">{{ old('description', $gamme->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <input type="text" id="icone" name="icone" value="{{ old('icone', $gamme->icone ?? 'fa-cube') }}"
                               placeholder="fa-cube"
                        ...
                        <div class="px-4 py-2 bg-gray-50 border border-gray-300 rounded-lg flex items-center">
                            <i class="fas {{ old('icone', $gamme->icone ?? 'fa-cube') }} text-amber-500" id="icone_preview"></i>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <input type="color" id="couleur" name="couleur" value="{{ old('couleur', $gamme->couleur ?? '#F59E0B') }}"
                               class="w-12 h-12 p-1 border border-gray-300 rounded-lg cursor-pointer">
                        <input type="text" name="couleur_text" value="{{ old('couleur', $gamme->couleur ?? '#F59E0B') }}"
                               class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                               id="couleur_text">
                    </div>
                ...
                <div>
                    ...
                    <input type="number" id="ordre" name="ordre" value="{{ old('ordre', $gamme->ordre ?? 0) }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent">
                    @error('ordre')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    <label class="flex items-center space-x-2">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $gamme->is_active) ? 'checked' : '' }}
