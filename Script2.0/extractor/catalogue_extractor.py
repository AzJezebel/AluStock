...
EXPECTED_HEADERS = {
    "REFERENCE": ["reference", "référence", "ref", "réf", "ref."],
    "KG/M": ["kg/m", "kgm", "kg m", "kg", "peso", "peso kg/m", "peso kg/m."],
    "IN": ["in", "in2", "in²"],
    "PERIM": ["perim", "périm", "perimeter", "périmètre", "per", "per.", "perimetro", "perímetro"],
}

# Tolérance d'appariement flou (0-1, plus haut = plus strict)
HEADER_MATCH_CUTOFF = 0.55

# Nombre minimum d'en-têtes reconnus dans une zone candidate pour la valider
# comme un véritable tableau de données. Filtre les schémas/plans dont le
# simple contour rectangulaire (ou hexagonal) ressemble géométriquement à
# une grille mais ne contient aucune donnée tabulaire.
MIN_HEADER_MATCHES_FOR_TABLE = 2

# Marge additionnelle appliquée à tous les crops exportés (schéma et
# tableau) : fraction de la taille de la boîte détectée, avec un minimum
# en pixels pour les petites boîtes.
...
def detect_table_boxes(gray: np.ndarray, tess_lang: str = "eng") -> list[tuple[int, int, int, int]]:
    ...
    # Filtre les faux positifs : un schéma/plan a un contour rectangulaire
    # (ou fait de segments droits pour un hexagone) qui ressemble à une
    # grille, mais ne contient pas les en-têtes REF/PESO/PER attendus.
    return [b for b in merged if _looks_like_data_table(gray, b, tess_lang)]
...
def _pad_box(box, img_shape, ratio: float = CROP_PADDING_RATIO, min_px: int = CROP_PADDING_MIN_PX):
    """Agrandit une boîte (x, y, w, h) d'une marge généreuse, sans dépasser
    les bords de l'image."""
    x, y, w, h = box
    img_h, img_w = img_shape[:2]
    pad_x = max(int(w * ratio), min_px)
    pad_y = max(int(h * ratio), min_px)
    nx = max(0, x - pad_x)
    ny = max(0, y - pad_y)
    nx2 = min(img_w, x + w + pad_x)
    ny2 = min(img_h, y + h + pad_y)
    return (nx, ny, nx2 - nx, ny2 - ny)


def _remove_page_border_frame(bin_img: np.ndarray) -> np.ndarray:
    """Efface les contours qui couvrent quasiment toute la page (cadre
    décoratif du cartouche catalogue) pour éviter qu'ils ne servent de pont
    et ne fusionnent tous les profilés de la page en un seul bloc."""
    h, w = bin_img.shape
    ...
    for c in contours:
        ...
        if cw > w * 0.85 and ch > h * 0.85:
            cv2.drawContours(cleaned, [c], -1, 0, thickness=25)
    return cleaned


def _looks_like_data_table(gray: np.ndarray, box, tess_lang: str) -> bool:
    """Valide qu'une zone candidate est bien un tableau de données (et pas un
    schéma dont le contour ressemble géométriquement à une grille), en
    vérifiant la présence d'au moins MIN_HEADER_MATCHES_FOR_TABLE en-têtes
    reconnus (REF., PESO/KG/M, PER., IN...)."""
    x, y, w, h = box
    ...
    for word in re.split(r"\s+", text.strip()):
        canon = _fuzzy_header_match(word)
        if canon:
            matched.add(canon)
    # Repasse aussi sur le texte complet pour les en-têtes en plusieurs mots
    # (ex: "peso kg/m") que le split mot-par-mot ne peut pas retrouver seul.
    for canon, variants in EXPECTED_HEADERS.items():
        for variant in variants:
            if " " in variant and variant in text_lower:
                matched.add(canon)

    return len(matched) >= MIN_HEADER_MATCHES_FOR_TABLE
...
def detect_profile_blocks(gray: np.ndarray, table_boxes):
    ...
    h, w = gray.shape
    bin_img = _binarize(gray)
    bin_img = _remove_page_border_frame(bin_img)
...
def ocr_table_rows(table_img: np.ndarray, tess_lang: str = "eng") -> list[TableRow]:
...
def process_pdf(pdf_path: str, out_dir: str, dpi: int, tess_lang: str, debug: bool,
    ...
    for page_num, img in pdf_to_cv_images(pdf_path, dpi=dpi):
        ...
        table_boxes = detect_table_boxes(gray, tess_lang=tess_lang)
        if not table_boxes:
        ...
        for profil_idx, r in enumerate(blocks, start=1):
            tx, ty, tw, th = _pad_box(r["table_box"], img.shape)
            bx, by, bw, bh = _pad_box(r["block_box"], img.shape)
...
def main():
    ...
    parser.add_argument(
        ...
        help=r"Chemin explicite vers tesseract.exe si absent du PATH "
             r"(ex: 'C:\Program Files\Tesseract-OCR\tesseract.exe')"
    )
